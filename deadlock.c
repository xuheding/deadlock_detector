...
struct source_type {
	...
	uint64 id;//线程id
	enum Type type;//该source的类型，因此，此结构体可以有两个用途，既可以放线程id，又可以放锁id
...
void unlock_after(uint64 thread_id, uint64 lockaddr) {
	...
	if (tg->locklist[idx].degress == 0) {//如果没进程申请该锁（入度为0）,就把该锁从locklist中清理。
...
//使用的时候，只要加入init_hook();start_check();就行了
#if 1  //debug
